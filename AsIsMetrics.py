...
orders_file = "master_suppliers.parquet"
...
orders = pd.read_parquet(os.path.join(folder, orders_file))
...
orders["deliver_date"] = pd.to_numeric(
    orders["deliver_date"],
    errors="coerce"
)

orders["deliver_date"] = pd.to_datetime(
    orders["deliver_date"].astype("Int64").astype(str),
    format="%Y%m%d",
    errors="coerce"
)

print("Datas válidas:", orders["deliver_date"].notna().sum())
print("Data mínima:", orders["deliver_date"].min())
print("Data máxima:", orders["deliver_date"].max())

orders["delivered_units"] = pd.to_numeric(
    orders["delivered_units"],
    errors="coerce"
).fillna(0)
...
orders["order_total_cost"] = (
    orders["delivered_units"] * (orders["unit_cost"] + transport_cost_per_unit)
)
...
stock[["total_order_cost", "total_ordered_units"]] = stock[
    ["total_order_cost", "total_ordered_units"]
].fillna(0)
...
valid_skus = sku_activity.loc[
    (sku_activity["total_forecast"] != 0) |
    (sku_activity["total_demand"] != 0),
    "sku"
]

stock = stock[stock["sku"].isin(valid_skus)].copy()

stock["stockout"] = np.maximum(
    stock["demand"] - stock["stock_on_hand"],
    0
)

stock["soh_end"] = np.maximum(
    stock["stock_on_hand"] - stock["demand"],
    0
)

stock["daily_holding_cost"] = (
    stock["soh_end"] * stock["unit_cost"] * (WACC / 365)
)
...
sku_metrics["total_cost"] = (
    sku_metrics["total_holding_cost"] +
    sku_metrics["total_order_cost"]
)
